...
2. **PCA projection regressed below the random projection.** Both K=32 and K=64 PCA collapse to a uniform-10 % baseline (exactly 1/10 — chance level for picking the right topic among 10).
   ...
   **v0.2.8 fix candidates:** (a) ~~PCA over individual docs (not centroids) so within-topic variation contributes to eigenvectors~~ — **tested 2026-05-14**, still 10.1 % chance level. The training-data hypothesis was wrong: PCA over 10 k docs (vs 100 centroids) doesn't move the result, so the issue isn't *what* PCA sees but *how the predictor scores against the result*. (b) **Orthogonalize predict() against `phi_last`** to suppress within-topic bias — **tested 2026-05-14**, helps some configurations (K=128 random: +19 pp, K=64 PCA: +12 pp) but regresses K=64 random catastrophically (57.2 % → 16.9 %). Workload-dependent fix, not universal. Reverted. (c) Different `M_low` initialization that doesn't put identity weight on self-features (the actual root cause: `M_low ≈ I + γ·(B⊗A)` after TD convergence gives `M_low·φ(A) ≈ φ(A) + γ·φ(B)` so the dominant direction points back at the current topic). Future work — likely requires a hybrid identity-plus-uniform-mixer init that's still bootstrap-correct.

   **API improvement shipped 2026-05-14:** `LowRankSrPredictor::with_pca_over_corpus(centroids, corpus_signatures)` takes the full doc corpus as PCA training data. Even though it doesn't move the bench numbers, it's the right interface for callers who *do* have richer training data than just event centroids (e.g., per-doc PCA in a real production deployment).
