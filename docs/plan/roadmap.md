...
- ✅ **QueryContext integration** (2026-05-14) — `ColdTier` trait moved to `primd-core` (so QueryContext can hold `Box<dyn ColdTier>` without circular deps). New `QueryContext::with_cold_tier()` builder; `QueryOutput::cold_results` surfaces cold-tier hits at every finalize when a cold tier is attached and non-empty. Auto-eviction (LRU on events) is still v0.4.1 — caller-driven `add_evicted` is the v0.4 entry point.
